# Course Creation Systems — WhatsApp Business listing
...
```bash
npm run pkg:grant -- --product course-creation --tier studio \
...
|---|---|
| Price | GHS 620 (Solo) |
| Image | `dist/mockups/course-creation/square.png` |
| Link | https://REPLACE-ME.example.com/products/course-creation |
...
The hundred hours, counted before you start.

A course is the most expensive digital product there is. A prompt pack is an afternoon; a course is a hundred hours, and almost nobody works that out before starting — which is why so many are abandoned halfway through module three, with the recording done and the workbook never written.
...
1. Open index.html and state the outcome first — what a finished student can do that they…
2. Pass the validation gate before touching curriculum.
...
The part people skip is Pass the validation gate before touching curriculum. If it fails, that is the cheapest failure available.
