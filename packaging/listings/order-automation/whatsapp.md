# E-Commerce Order Automation — WhatsApp Business listing
...
```bash
npm run pkg:grant -- --product order-automation --tier studio \
...
|---|---|
| Price | GHS 620 (Solo) |
| Image | `dist/mockups/order-automation/square.png` |
| Link | https://paystack.shop/mosh-digital-studios/products/order-automation |
...
A store's order desk is the part that scales worst. A listing is written once; an order is processed every single time, and the minutes are invisible — nobody invoices you for reading the order, checking the payment, picking, packing, buying postage, answering 'where is it?', or reconciling the payout.
...
The part people skip is Read the total hours. It is usually the moment the case makes itself.
